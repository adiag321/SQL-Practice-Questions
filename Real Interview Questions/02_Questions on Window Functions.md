...
SELECT 
    ...
    v1.video_id,
    v1.view_count
FROM youtube_videos v1
WHERE (
    -- For this video, count how many other videos in the same channel have strictly more views
    SELECT COUNT(*)
    FROM youtube_videos v2
    WHERE v2.channel_id = v1.channel_id
      AND v2.view_count > v1.view_count
) <= 5
...
Solution: 
-- Active Subscribers mean subscribers with recent "last_update_date"
```sql
with recent_bill as 
(
    SELECT
