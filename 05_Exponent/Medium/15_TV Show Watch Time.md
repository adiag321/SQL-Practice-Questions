...
with cte as (select
*,
lead(watch_hours) over(partition by viewer_id
        order by year, month ) as nxt_mnth_watch_hr
from watch_time
),
cte2 as (select
viewer_id,
...
watch_hours,
nxt_mnth_watch_hr,
case when (nxt_mnth_watch_hr - watch_hours) > 0 then 1 else 0 end as mom_incr
from cte
),
cte3 as (select
*,
lead(mom_incr) over(partition by viewer_id
        order by year, month ) as mom_incr_2_mnth,
lead(mom_incr,2) over(partition by viewer_id
        order by year, month ) as mom_incr_3_mnth
from cte2
)
select
distinct viewer_id
from cte3
where mom_incr = 1
and mom_incr_2_mnth = 1
and mom_incr_3_mnth = 1;
```
